...
from .models import KPSS_TYPE_CHOICES, Question, TgExam
from .push import send_tg_exam_announcement_push
...
def _tg_exam_from_post(request: HttpRequest, item: TgExam | None) -> TgExam:
    ...
    try:
        obj.duration_minutes = max(
            30,
            int(request.POST.get("duration_minutes") or 120),
        )
    except (TypeError, ValueError):
        obj.duration_minutes = 120
    return obj
...
def panel_tg_exam_edit(
    ...
    if request.method == "POST":
        ...
        if action == "generate":
            ...
            if not obj.title or not obj.start_at or not obj.end_at:
                ...
                except ExamGeneratorError as exc:
    ...
    expected_total = ExamGeneratorService().slot_count()
...
def panel_tg_exam_replace_question(
    ...
    if exam.is_published:
        messages.warning(request, "Yayındaki deneme düzenlenemez — önce yayından kaldırın.")
        return redirect("panel_tg_exam_edit", exam_id=exam.pk)
...
def panel_tg_exam_publish(request: HttpRequest, exam_id: int) -> HttpResponse:
    ...
    expected = ExamGeneratorService().slot_count()
    ...
    if send_push:
        ...
        if result.ok:
            messages.success(
                request,
                f"Duyuru bildirimi gönderildi ({result.success} cihaz).",
            )
        else:
            messages.warning(request, result.error or "Bildirim gönderilemedi.")
