"""TG deneme REST API — /api/v1/tg-exams/"""
...
from content.auth import get_user_from_request
from content.models import Question, TgExam, TgExamAttempt
from content.serializers import QuestionSerializer
from content.test_grouping import order_questions_keeping_scenarios

from .constants import VALID_KPSS_TYPES
from .grading import grade_attempt, kpss_net
from .ranking import finalize_due_tg_exams, refresh_exam_rankings
from .announcements import dispatch_due_tg_exam_announcements
from .serializers import exam_to_dict


def _run_tg_exam_background_tasks(*, send_push: bool = True) -> None:
    dispatch_due_tg_exam_announcements(send_push=send_push)
...
class TgExamQuestionsView(APIView):
    ...
    def get(self, request, exam_id: int):
        ...
        if not question_ids:
            return Response(
                {"detail": "Deneme soruları henüz tanımlanmadı."},
                status=409,
            )
        ...
        # TG denemede havuz sorularının ÖSYM etiketi kullanıcıya gösterilmez.
        for row in data:
...
class TgExamProgressView(APIView):
    ...
    def post(self, request, exam_id: int):
        ...
        current_index = int(
            request.data.get("current_index")
            or request.data.get("currentIndex")
            or 0
        )
        elapsed = int(
